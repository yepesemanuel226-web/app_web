...
export function Catalog() {
  ...
  const handlePurchase = async (e: React.FormEvent) => {
    ...
    const total = (selectedBook.sale_price || 0) * quantity;

    const { error } = await supabase.from('sales').insert([{
      user_id: user.id,
      book_id: selectedBook.id,
      quantity,
      unit_price: selectedBook.sale_price,
      total_amount: total,
          ...
          {filteredBooks.map((book) => (
            <Card key={book.id}
              ...
              {book.sale_price && book.stock_sale > 0 && (
                <p className="text-[#E8A020] font-semibold">${book.sale_price.toLocaleString('es-CO')}</p>
              )}
            </Card>
          ))}
        </div>
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div className="md:col-span-2">
                  ...
                  {selectedBook.stock_sale > 0 && selectedBook.sale_price && (
                    <p className="text-3xl font-bold text-[#E8A020] mb-4">${selectedBook.sale_price.toLocaleString('es-CO')}</p>
                  )}
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    ...
                    {selectedBook.stock_sale > 0 && <div><p className="text-gray-600">Stock venta</p><p className="font-semibold text-[#1A3A5C]">{selectedBook.stock_sale} disponibles</p></div>}
                  </div>
                </div>
              </div>
              ...
              <div className="border-t border-gray-200 pt-6">
                ...
                {activeTab === 'buy' && selectedBook.stock_sale > 0 && (
                      ...
                      <>
                        <div className="flex items-center gap-4 mb-6">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-3xl font-bold text-[#388E3C]">${((selectedBook.sale_price || 0) * quantity).toLocaleString('es-CO')}</p>
                          </div>
                        </div>
                      ...
                      <form onSubmit={handlePurchase} className="space-y-4">
                        ...
                        <div className="bg-white rounded-lg p-4 border border-gray-300">
                          ...
                          <div className="space-y-2 text-sm">
                            ...
                            <div className="flex justify-between"><span className="text-gray-600">Precio unitario:</span><span className="font-medium">${selectedBook.sale_price?.toLocaleString('es-CO')}</span></div>
                            <div className="border-t border-gray-200 pt-2 mt-2 flex justify-between">
                              <span className="font-bold text-[#1A3A5C]">Total:</span>
                              <span className="font-bold text-[#388E3C] text-xl">${((selectedBook.sale_price || 0) * quantity).toLocaleString('es-CO')}</span>
                            </div>
                          </div>
                        </div>
